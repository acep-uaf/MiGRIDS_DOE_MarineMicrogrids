...
class getMinSrc:
    ...
    def getMinSrc(self, SO, calcFuture = False, nearTerm = False):
        ...
        if calcFuture and not nearTerm:
            ...
            self.minSrcToStay = SO.futureLoad * SO.DM.minSRC + sum([a * b for a, b in zip(SO.futureWindNearTerm, SO.WF.wtgMinSrcCover)])  # catch zero instances of wind power with max statement. in this case, nonFirmWtgP will be 0 as well.
            self.minSrcToSwitch = self.minSrcToStay * self.minSRCToSwitchRatio
